function iOSRedirect(variables){
    ...
    if(!urlRegExp.test(variables.redirectTo)){
        throw new Error("iOS-Redirecter-Plugin - URL is not a valid one");
    }
    var checkArray = new Array();
    if(variables.iPad){
        checkArray.push('iPad');
    }
    if(variables.iPhone){
        checkArray.push('iPhone');
    }
    var checkString = checkArray.join('|');
    if(checkString){
        var checkRegExp = new RegExp(checkString,'g');
        if(checkRegExp.test(navigator.userAgent)){
            location.href = variables.redirectTo;
        }
    }
}